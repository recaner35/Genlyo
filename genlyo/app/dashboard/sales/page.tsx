...
export default function DashboardHomePage() {
  ...
  const fetchRealizedSales = async () => {
    try {
       // 🚀 DÜZELTME 2: Parametrenin sonuna "Date.now()" ekleyerek tarayıcıyı kandırıp hep en güncel veriyi çekiyoruz.
  ...
  const handleQuickSave = async () => {
    ...
    try {
      ...
      // 🚀 DÜZELTME 3: Sadece "res.ok" değil, işlenen satır sayısının sıfırdan büyük olduğundan emin oluyoruz!
      if (res.ok && resData.count > 0) {
        setQuickRevenue("");
        alert("✅ Bugünün cirosu başarıyla kaydedildi!");
        fetchRealizedSales(); // Ekrandaki barı anında doldur
      } else {
